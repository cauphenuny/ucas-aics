...
class VGG19(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(*[a[x] for x in range(3)])
        # TODO: 定义self.layer2为第4层卷积后对应的特征
        self.layer2 = nn.Sequential(*[a[x] for x in range(3, 8)])
        # TODO: 定义self.layer3为第8层卷积后对应的特征
        self.layer3 = nn.Sequential(*[a[x] for x in range(8, 17)])
        # TODO: 定义self.layer4为第12层卷积后对应的特征
        self.layer4 = nn.Sequential(*[a[x] for x in range(16, 22)])
        """
(Pdb) print(a)
VGG(
  (features): Sequential(
...
if __name__ == '__main__':
    ...
    loss_func = torch.nn.MSELoss().cpu()
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            if i % 10 == 0:
                ...
                save_image(
                    torch.cat([image_g, image_c], dim=3), f"./out/train/{j}_{i}.jpg"
                )
        j += 1
