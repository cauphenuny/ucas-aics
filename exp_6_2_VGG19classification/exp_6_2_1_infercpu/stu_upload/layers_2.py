...
class ConvolutionalLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        self.input_pad[
            :,
            :,
            self.padding : self.input.shape[2] + self.padding,
            self.padding : self.input.shape[3] + self.padding,
        ] = self.input
...
class FlattenLayer(object):
    def __init__(self, input_shape, output_shape):  # 扁平化层的初始化
        ...
        print(
            "\tFlatten layer with input shape %s, output shape %s."
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        self.input = np.transpose(input, [0, 2, 3, 1])  # [N, height, width, channel]
